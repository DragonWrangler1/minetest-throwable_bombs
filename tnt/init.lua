...
local S = minetest.get_translator("tnt_redo")
...
local tnt_radius = tonumber(minetest.settings:get("tnt_radius") or 20)
...
function   tnt_redo.burn(pos, nodename)
...
local function tnt_explode(pos, radius, ignore_protection, ignore_on_blast, owner, explode_center)
	...
	local count = 0
	local c_tnt
	local c_tnt_burning = minetest.get_content_id("tnt_redo:tnt_burning")
	local c_tnt_boom = minetest.get_content_id("tnt_redo:boom")
	local c_air = minetest.CONTENT_AIR
	local c_ignore = minetest.CONTENT_IGNORE
        if minetest.get_modpath("fire") then
		c_tnt = minetest.get_content_id("tnt_redo:tnt")
	else
...
function tnt_redo.boom(pos, def)
	def = def or {}
	def.radius = def.radius or 6
	def.damage_radius = def.damage_radius or def.radius * 4
	local meta = minetest.get_meta(pos)
	local owner = meta:get_string("owner")
	if not def.explode_center and def.ignore_protection ~= true then
		minetest.set_node(pos, {name = "tnt_redo:boom"})
	end
...
minetest.register_node("tnt_redo:boom", {
	drawtype = "airlike",
...
minetest.register_node("tnt_redo:gunpowder", {
	...
	on_punch = function(pos, node, puncher)
		if puncher:get_wielded_item():get_name() == "default:torch" then
			minetest.set_node(pos, {name = "tnt_redo:gunpowder_burning"})
			default.log_player_action(puncher, "ignites tnt_redo:gunpowder at", pos)
		end
	end,
...
minetest.register_node("tnt_redo:gunpowder_burning", {
	drawtype = "raillike",
	paramtype = "light",
	sunlight_propagates = true,
	walkable = false,
...
minetest.register_craft({
	output = "tnt_redo:gunpowder 5",
	type = "shapeless",
	recipe = {"nether:nether_lump", "default:gravel"}
})

minetest.register_craftitem("tnt_redo:tnt_stick", {
	description = S("TNT Stick"),
	inventory_image = "tnt_tnt_stick.png",
	groups = {flammable = 5},
})
	minetest.register_craft({
		output = "tnt_redo:tnt_stick 2",
		recipe = {
			{"tnt_redo:gunpowder", "", "tnt_redo:gunpowder"},
			{"tnt_redo:gunpowder", "default:paper", "tnt_redo:gunpowder"},
			{"tnt_redo:gunpowder", "", "tnt_redo:gunpowder"},
		}
	})
	...
	minetest.register_craft({
		output = "tnt_redo:tnt",
		recipe = {
			{"tnt_redo:tnt_stick", "tnt_redo:tnt_stick", "tnt_redo:tnt_stick"},
			{"tnt_redo:tnt_stick", "tnt_redo:tnt_stick", "tnt_redo:tnt_stick"},
			{"tnt_redo:tnt_stick", "tnt_redo:tnt_stick", "tnt_redo:tnt_stick"}
		}
	})
	...
	minetest.register_abm({
		label = "TNT ignition",
		nodenames = {"group:tnt", "tnt_redo:gunpowder"},
		neighbors = {"fire:basic_flame", "default:lava_source", "default:lava_flowing"},
		interval = 4,
		chance = 1,
		action = function(pos, node)
			tnt_redo.burn(pos, node.name)
		end,
	})

function tnt_redo.register_tnt(def)
	local name
	if not def.name:find(':') then
		name = "tnt_redo:" .. def.name
	else
		name = def.name
	...
	local tnt_burning = def.tiles.burning or def.name .. "_top_animated.png"
	...
	minetest.register_node("tnt_redo:tnt_burning", {
		tiles = {
			{
